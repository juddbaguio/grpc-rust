...
use repo::{
    connect_db, movie_repo,
    ports::{MovieRepo, UserRepo},
    user_repo,
};
...
pub async fn run() -> Result<(), Box<dyn Error>> {
    ...
    let user_service = user_service::new(UserRepo::Concrete(user_repo));
    let movie_service = movie_service::new(MovieRepo::Concrete(movie_repo));
